...
class UserService(
    ...
    fun deleteAvatar(id: Long): Boolean {
        val user = findByIdOrNull(id) ?: return false
        return avatarService.delete(user.avatar)
    }

    //TESTE buscar ou gerar avatar usando API externa.
    fun generateAvatar(id: Long): String? {
        val user = findByIdOrThrow(id)
        avatarService.getExternalAvatar(user)?.let {
            saveAvatar(id, it)
            return "Avatar gerado com sucesso!"
        }
        return null
    }
